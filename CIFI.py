...
class App(ttk.Frame):
    def __init__(self, parent):
        ...
        # Make the app responsive (This might not be needed)
        for index in [0, 1, 2]:
    ...
    def import_file(self):
        file_path = askopenfilename(defaultextension=".pkl", filetypes=[("Text Documents", "*.pkl"), ("All Files", "*.*")])
        if file_path != '':
            with open(file_path, 'rb') as file:
                self.player = pickle.load(file)
                print(f'Player loaded from {file_path}')
    ...
    def export_file(self):
        file_path = asksaveasfilename(confirmoverwrite=False, initialfile='save.pkl', defaultextension=".pkl", filetypes=[("All Files", "*.*"), ("Text Documents", "*.pkl")])
        if file_path != '':
            with open(file_path, 'wb') as file:
                pickle.dump(self.player, file, pickle.HIGHEST_PROTOCOL)
                print(f'Player saved to {file_path}')

    def setup_main(self):
        option_tab = ttk.Frame(self.tabControl)

        # Import Button
        button2 = ttk.Button(option_tab, text="Import", command=self.import_file)
        button2.grid(row=0, column=0, padx=5, pady=10, sticky="nsew")

        # Export Button
        button = ttk.Button(option_tab, text="Export", command=self.export_file)
        button.grid(row=1, column=0, padx=5, pady=10, sticky="nsew")

        self.tabControl.add(option_tab, text='Main')

    def validate_int(self, value, acttyp):
        # Example - https://stackoverflow.com/questions/4140437/interactively-validating-entry-widget-content-in-tkinter
        if acttyp == '1':  # insert
    ...
    def set_int(self, value):
        # Connect Var - https://stackoverflow.com/questions/2524031/how-to-connect-a-variable-to-entry-widget
        # https://www.tutorialspoint.com/how-to-connect-a-variable-to-the-tkinter-entry-widget
        # https://www.pythontutorial.net/tkinter/tkinter-entry/
        self.player.player['Level'] = int(value)

    def setup_player(self):
        player_tab = ttk.Frame(self.tabControl)

        widgets_frame = ttk.Frame(player_tab, padding=(0, 0, 0, 10))
        widgets_frame.grid(row=0, column=1, padx=10, pady=(30, 10), sticky="nsew", rowspan=3)
        widgets_frame.columnconfigure(index=0, weight=1)

        # Level
        test = tk.Label(player_tab, text='Level:')
        test.grid(row=0, column=0, padx=10, pady=10)
        levelvar = tk.StringVar(player_tab, str(self.player.player['Level']))
        levelvar.trace('w', self.set_int)
        entry = ttk.Entry(player_tab, validate="key", textvariable=levelvar)
        entry['validatecommand'] = (entry.register(self.validate_int), '%P', '%d')
        entry.grid(row=0, column=1, padx=10, pady=10,  sticky="ew")

        self.tabControl.add(player_tab, text='Player')

    def setup_token(self):
        token_tab = ttk.Frame(self.tabControl)
        self.tabControl.add(token_tab, text='Tokens')

    def setup_diamond(self):
        diamond_tab = ttk.Frame(self.tabControl)
        self.tabControl.add(diamond_tab, text='Diamonds')

    def setup_gen(self):
        gen_tab = ttk.Frame(self.tabControl)
        self.tabControl.add(gen_tab, text='Gens')

    def setup_ship(self):
        ship_tab = ttk.Frame(self.tabControl)
        self.tabControl.add(ship_tab, text='Fleet')

    def setup_tech(self):
        tech_tab = ttk.Frame(self.tabControl)
        self.tabControl.add(tech_tab, text='Tech Upgrades')

    def setup_loop(self):
        loop_tab = ttk.Frame(self.tabControl)
        self.tabControl.add(loop_tab, text='Loop Mods')

    def setup_shard(self):
        shard_tab = ttk.Frame(self.tabControl)
        self.tabControl.add(shard_tab, text='Shards')

    def setup_research(self):
        research_tab = ttk.Frame(self.tabControl)
        self.tabControl.add(research_tab, text='Research')

    def setup_academy(self):
        academy_tab = ttk.Frame(self.tabControl)
        #
        # This is an example from the theme (It shouldn't be using self like it is)
        #
        ...
        self.check_frame = ttk.LabelFrame(academy_tab, text="Checkbuttons", padding=(20, 10))
        ...
        # Separator
        self.separator = ttk.Separator(academy_tab)
        self.separator.grid(row=1, column=0, padx=(20, 10), pady=10, sticky="ew")

        # Create a Frame for the Radiobuttons
        self.radio_frame = ttk.LabelFrame(academy_tab, text="Radiobuttons", padding=(20, 10))
        ...
        self.widgets_frame = ttk.Frame(academy_tab, padding=(0, 0, 0, 10))
        ...
        # Panedwindow
        self.paned = ttk.PanedWindow(academy_tab)
        ...
        # Sizegrip
        self.sizegrip = ttk.Sizegrip(academy_tab)
        self.sizegrip.grid(row=100, column=100, padx=(0, 5), pady=(0, 5))

        #
        # End of Example
        #
        self.tabControl.add(academy_tab, text='Academy')
